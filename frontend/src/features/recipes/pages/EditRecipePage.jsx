...
export default function EditRecipePage() {
  ...
  // Detectar si venimos del admin
  const fromAdmin = location.state?.fromAdmin || location.pathname.includes('/admin');
  ...
  useEffect(() => {
    if (recipeData?.data) {
      ...
      reset({
        ...
        steps: recipe.steps?.length > 0 ? recipe.steps.map(step => ({
  ...
  const updateRecipeMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Si venimos del admin, volvemos al panel admin, sino a la receta
      if (fromAdmin) {
    ...
    onError: (error) => {
      toast.error(error.response?.data?.error?.message || 'Error al actualizar receta');
    },
  });
  ...
  const onSubmit = async (data) => {
    let imageUrl = recipeData?.data?.imageUrl;
    ...
    updateRecipeMutation.mutate(recipePayload);
  };
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6">
        Editar Receta{recipe?.title ? `: ${recipe.title}` : ''}
      </h1>
